...
def train(args, train_dataset, model, tokenizer):
    ...
    if args.do_freeze:
        ...
        # Unfreeze parameters in the qa_outputs layer
...
def evaluate(args, model, tokenizer, prefix=""):
    ...
    if args.version_2_with_negative:
        output_null_log_odds_file = os.path.join(args.predict_dir, "null_odds_{}.json".format(prefix))
    else:
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="The directory where the model checkpoints will be written.",
    )
    parser.add_argument(
        "--predict_dir",
        default=None,
    ...
    parser.add_argument(
        ...
        default=None,
        type=str,
        help="The directory where the pretrained model was saved.",
    )
    parser.add_argument(
        "--data_dir",
        default=None,
    ...
    parser.add_argument("--do_train", action="store_true", help="Whether to run training from the scratch.")
    parser.add_argument("--do_eval", action="store_true", help="Whether to run eval on the dev set.")
    parser.add_argument("--do_freeze", action="store_true", help="Whether to run training only for the head.")
    parser.add_argument(
    ...
    if args.do_train:
        if os.path.exists(args.model_dir) and os.listdir(args.model_dir):
            model = AutoModelForQuestionAnswering.from_pretrained(args.model_dir)
            tokenizer = AutoTokenizer.from_pretrained(args.model_dir, do_lower_case=args.do_lower_case, use_fast=False)
            model.to(args.device)
